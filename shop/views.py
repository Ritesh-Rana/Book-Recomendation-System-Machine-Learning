...
def recommend(request):
    ...
    product_list=list(Product.objects.filter(id__in = pred_idxs_sorted,))
    print(product_list)
    random.shuffle(product_list)
    return render(request,'shop/recommend.html',{'product_list':product_list[:10]})
...
def product_list(request, category_slug=None):
    ...
    products = Product.objects.filter(available=True)[:100]
    search_term=''
    if category_slug:
        category = get_object_or_404(Category, slug=category_slug)
        products = Product.objects.filter(category=category)[:100]
